        ...
        loop {
            unsafe {
                ...
                while !hwnd.is_invalid() && check_count < 15 {
                    ...
                    let is_nectar = process_id == my_process_id || class_str.contains("Nectar");
                    ...
                    if is_nectar || !is_visible || is_iconic || is_cloaked || !has_valid_rect {
        ...
        if let Some(app_handle) = MOUSE_HOOK_APP_HANDLE.get() {
            ...
            if let Some(dock_win) = app_handle.get_webview_window("dock") {
                if dock_win.is_visible().unwrap_or(false) {
                    ...
                    if MH_LAST_DOCK_IGNORE.swap(1, Ordering::Relaxed) != 1 {
            ...
            // is forced during fullscreen.
            {
                if let Some(main_win) = app_handle.get_webview_window("main") {
                    if main_win.is_visible().unwrap_or(false) {
                        ...
                        if MH_LAST_MAIN_IGNORE.swap(1, Ordering::Relaxed) != 1 {
...
pub fn register_appbar(window: tauri::WebviewWindow) {
    ...
    if let Some(monitor) = crate::monitors::monitor_for_window_label(window.app_handle(), crate::types::WindowKind::Notch, &label) {
        ...
        let nectar_scale = crate::utils::get_nectar_scale(window.app_handle());
        let ph = ((420.0 * nectar_scale) * scale) as i32;
        ...
        unsafe {
            ...
            let target_width = m_size.width as i32;
            ...
            if GetWindowRect(hwnd, &mut current_rect).is_ok() {
                ...
                if current_rect.left == target_left
                    && current_rect.top == target_top
            ...
            if !already_positioned {
                let _ = SetWindowPos(hwnd, None, target_left, target_top, target_width, ph, SWP_NOZORDER | SWP_NOACTIVATE | SWP_FRAMECHANGED);
            }
...
fn register_dock_appbar_inner(window: tauri::WebviewWindow, attempt: i32) {
    ...
    if let Some(monitor) = crate::monitors::monitor_for_window_label(window.app_handle(), crate::types::WindowKind::Dock, &label) {
        ...
        let pr = ((56.0 * nectar_scale) * scale) as i32;
...
pub unsafe extern "system" fn enum_windows_proc(hwnd: HWND, lparam: LPARAM) -> BOOL {
    ...
    if IsWindowVisible(hwnd).as_bool() {
        ...
        if len > 0 {
            ...
            if title == "Program Manager" || title == "nectar" || title == "Nectar Dock" {
                return true.into();
            }
...
pub fn setup_display_change_monitor(app_handle: AppHandle) {
    ...
    std::thread::spawn(|| {
        unsafe {
            ...
            let class_name = windows::core::PCSTR(c"NectarDisplayMonitor".as_ptr() as *const u8);
